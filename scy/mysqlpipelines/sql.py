...
cnx = MySQLdb.connect(user=MYSQL_USER, password=MYSQL_PASSWORD, host=MYSQL_HOSTS, database=MYSQL_DB,charset="utf8")
cur = cnx.cursor()
...
class Sql:
    ...
    def insert_dd_name(cls, xs_name, xs_author, category, name_id):
        ...
        sql = 'INSERT INTO dd_name (`xs_name`, `xs_author`, `category`, `name_id`) VALUES(\"{0}\", \"{1}\",\"{2}\", \"{3}\")' 
        sql = sql.format(xs_name,xs_author,category,name_id)
        #(xs_name, xs_author, category, name_id)
        #sql = 'INSERT INTO dd_name (`xs_name`, `xs_author`, `category`, `name_id`) VALUES("nide", "bbb","ccc", "ddd")' 
        sql = sql.decode("gbk").encode("utf-8")
        cur.execute(sql)
        cnx.commit()"""
